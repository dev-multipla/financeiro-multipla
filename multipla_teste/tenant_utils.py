...
def ensure_tenant_db(empresa):
    ...
    if alias not in settings.DATABASES:
        default_cfg = settings.DATABASES['default'].copy()
        settings.DATABASES[alias] = {
            'ENGINE': default_cfg['ENGINE'],
            'NAME': f'multipla_financeiro_tenant_{empresa.id}',
            'USER': default_cfg['USER'],
            'PASSWORD': default_cfg['PASSWORD'],
            'HOST': default_cfg['HOST'],
            'PORT': default_cfg['PORT'],
            'OPTIONS': default_cfg.get('OPTIONS', {}),
            'CONN_MAX_AGE': default_cfg.get('CONN_MAX_AGE', 60),
        }
...
def aggregate_across_tenants(model_class, queryset_fn, include_empresa_info=True):
    """
    Agrega dados de todos os tenants disponíveis
    
    Args:
        model_class: Classe do modelo Django
        queryset_fn: Função que recebe queryset e retorna resultado
        include_empresa_info: Se deve incluir informações da empresa no resultado
        
    Returns:
        list: Lista de resultados por tenant
        
    Example:
        def count_clientes(qs):
            return qs.count()
            
        results = aggregate_across_tenants(Cliente, count_clientes)
        # [{'empresa': <Empresa>, 'result': 15}, ...]
    """
    results = []
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_tenant, empresa) for empresa in empresas]
...
def execute_on_all_tenants(operation_fn, include_default=False):
    ...
    Example:
        def fix_data(empresa):
            # Alguma operação de correção
            return "Corrigido"
            
        results = execute_on_all_tenants(fix_data)
    """
    results = {}
